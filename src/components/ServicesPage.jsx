...
export default function ServicesPage() {
    ...
    useEffect(() => {
        if (modalRef.current) {
            const modal = new bootstrap.Modal(modalRef.current, { keyboard: false, backdrop: false });
            if (isServiceModalOpen) modal.show(); else modal.hide();
    ...
    useEffect(() => {
        if (addressModalRef.current) {
            const addressModal = new bootstrap.Modal(addressModalRef.current, { keyboard: false, backdrop: false });
            if (isAddressModalOpen) addressModal.show(); else addressModal.hide();
            return () => { addressModal.hide(); addressModal.dispose(); };
        }
    }, [isAddressModalOpen]);

    useEffect(() => {
        if (detailsModalRef.current) {
            const detailsModal = new bootstrap.Modal(detailsModalRef.current, { keyboard: false, backdrop: false });
            if (isDetailsModalOpen) detailsModal.show(); else detailsModal.hide();
            return () => { detailsModal.hide(); detailsModal.dispose(); };
        }
    }, [isDetailsModalOpen]);
    ...
    const handleCardClick = (service) => {
        ...
        const photos = getPhotos(service);
    ...
    const handleClick = () => {
        const encodedText = encodeURIComponent(`Здравствуйте! Меня заинтересовало данное объявление: УСЛУГИ ${selectedService['Название']}`);
        ...
        <>
            <style>
                {`
                    ...
                    #addressModal { z-index: 1060; }
                    #addressModal:not(.show), #serviceModal:not(.show), #detailsModal:not(.show) { display: none !important; pointer-events: none; }
                    #detailsModal { z-index: 1070; }
                    #serviceModal .modal-content { pointer-events: ${isAddressModalOpen || isDetailsModalOpen ? 'none' : 'auto'}; }
                    #serviceModal.blurred .modal-content { filter: blur(5px); }
                ...
                <div className="app-container h-75 w-100 z-1 d-flex flex-column justify-content-center align-items-center">
                    <div className="d-flex w-100 justify-content-center align-items-center flex-column gap-5 buttons-cont">
                        {isLoading ? (
                            ...
                            <div className="w-100 h-100 d-flex justify-content-center align-items-center text-danger">Ошибка: {error}</div>
                        ) : data.length === 0 ? (
                            ...
                            <div className="catalog-cards-cont">
                                {data.map((service, key) => {
                                    ...
                                    return (
                                        <div key={key} className="catalog-card cursor-pointer" onClick={() => handleCardClick(service)}>
            ...
            <div className={`modal fade ${isServiceModalOpen ? 'show' : ''} ${isAddressModalOpen || isDetailsModalOpen ? 'blurred' : ''}`} id="serviceModal" ref={modalRef} tabIndex="-1" aria-labelledby="serviceModalLabel" aria-hidden={!isServiceModalOpen}>
                        ...
                        {selectedService && (
                            <>
                                <div className="modal-body modal-body-scrollable text-center">
                                    ...
                                    <div className="services-bonus text-white mt-3">{selectedService['Акция']}</div>
                                    <div className="w-100 d-flex justify-content-between p-3">
                                        <button className="text-white home-btn" onClick={handleOpenAddressModal}>АДРЕС</button>
                                        <button className="text-white home-btn" onClick={handleOpenDetailsModal}>ПОДРОБНЕЕ</button>
                                    </div>
                                </div>
            ...
            {/* Оставшиеся модальные окна (addressModal и detailsModal) опущены для краткости, они не требуют изменений */}
        </>
    );
}